...
<html>
    ...
    <body>
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <c:choose>
                        <c:when test="${book != null}">
                            <ul class="navbar-nav mr-1">
                                <li class="nav-item flex-bookname">
                                    <a class="nav-link" href="${pageContext.servletContext.contextPath}/Home/ProfileDetails.jsp">${book.bookname}</a>
                                </li>
                                <li class="nav-item">
        ...
        <main>
            <div class="container">
                <c:set var="book" value="<%= BookWS.read(request.getParameter("id"))%>" />
                <form id="edit-book-form" class="custom-form" method="POST" enctype="multipart/form-data" action="${pageContext.servletContext.contextPath}/books">
                    <h2 class="custom-form-title">Edit Book</h2>

                    <input type="hidden" name="method" value="PUT">
                    <input type="hidden" name="id" value="${book.id}">
                    <input class="custom-form-input" type="text" name="title" value="${book.title}" placeholder="Title" required />
                    <input class="custom-form-input" type="text" name="author" value="${book.author}" placeholder="Author" required />
                    <input class="custom-form-input" type="text" name="information" value="${book.information}" placeholder="Information" required />


                    <select name="categoryID" class="custom-form-select-input" required>
                        <option value="" selected>Category...</option>
                        <c:forEach var="category" items="<%= CategoryWS.read()%>">
                            <option value="${category.id}" ${category.id == book.categoryID.id ? "selected" : ""}>${category.name}</option>
                        </c:forEach>
                    </select>

                    <input class="custom-form-input" type="number" name="price" value="${book.price}" placeholder="Price" required />

                    <input type="file" name="thumbnail" class="custom-form-file-input" />

                    <input class="custom-form-input" type="number" name="count" value="${book.count}" placeholder="Count" required />

                    <button class="custom-form-button">Edit</button>
                    <a class="custom-form-bottom-link" href="${pageContext.servletContext.contextPath}/Books">Go Back</a>
                </form>
            </div>
        </main>
